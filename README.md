...
| **Final** | best-ever locks (f2–f8), **signal push** (f1) | — | The lock-in: seven functions submit their verified best coordinates, guaranteed by the determinism results. f1 takes the one deliberate gamble — a further step along the line that returned −0.0053 → +0.0027 → +0.0315, chasing the ~10×-per-step growth instead of banking 0.0315. |

Current best scores per function:

| Func | Best so far | At input | Acquisition that found it |
|---|---|---|---|
| f1 | **+0.0315** | (0.43, 0.455) | signal_step (W12) — 11.7× the W11 reading; still climbing along the line |
